...
**Правило ошибок предметной области:** sentinel-ошибки (`usecases.ErrNoteNotFound`,
`usecases.ErrValidation`) объявлены в пакете `usecases` и переиспользуются как
`repository`, так и `handlers` — это тоже пример инверсии зависимостей, а не
нарушение слоистости. Например, `repository.GetNote` при `sql.ErrNoRows`
возвращает `errors.Join(usecases.ErrNoteNotFound, sql.ErrNoRows)`, потому что
`repository` зависит от контрактов `usecases` (включая ошибки), а не наоборот.
`handlers` затем проверяет `errors.Is(err, usecases.ErrNoteNotFound)`, чтобы
выбрать HTTP-статус. Так use cases остаются единственным местом, где объявлено
бизнес-значение ошибки, а внешние слои лишь ссылаются на него.
